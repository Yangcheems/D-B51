"""
===============================================================================
Goal-Seeking Navigation App/Pipeline
===============================================================================
    ...
    def select_next_frontier(self, frontiers: List[Tuple[int, int]], ogm: OccupancyGridMap,
        ...
        if goal_pos is not None:
            ...
            for fx, fy in frontiers:
                ...
                score = -dist_to_goal + 0.3 * (-dist_to_robot)
...
def get_lidar_scan() -> Scan:
    if SIM_MODE:
        ...
        import robot_serial_interface_enhanced as robot_serial
        return robot_serial.get_lidar_scan(timeout=2.0, min_points=20)
...
class AppPipeline:
    ...
    def tick(self) -> bool:
        ...
        if icp_pose is not None and prev_pts_for_plot is not None and prev_pose_for_tf is not None:
            ...
            tx = icp_pose.x - prev_pose_for_tf.x;
            ty = icp_pose.y - prev_pose_for_tf.y
        ...
        if self.planner is not None and self.step - self.last_plan_step >= self.plan_interval:
            try:
                ...
                if new_path is not None:
                    ...
                    logger.debug(
                        f"[PLANNING] Replanning path, step count: {self.step}, number of path points: {len(new_path)}")
            except Exception as e:
                logger.debug(f"[PLANNING] Planning failed: {e}")
        ...
        if self._at_goal(pose):
            ...
            logger.info("Reached goal; stopping.")
            plt.show(block=True);
            return False
        # Decide & act
        if self.app_cfg.mode == "GOALSEEKING":
            ...
            setpoint = self.controller.compute(pose, scan, self.mapper, self.goal.cell,
                                               self.world.cell_size_m)  # type: ignore[attr-defined]
            if setpoint is None:
...
class ManualController:
    ...
    def decide(self, pose: Pose, scan: Scan, world: _MazeBase, goal: Goal) -> Pose:
        ...
        if key == "up" and can_move: return Pose(pose.x + step_m * math.cos(pose.theta),
                                                 pose.y + step_m * math.sin(pose.theta), pose.theta)
        if key == "down" and can_move: return Pose(pose.x - step_m * math.cos(pose.theta),
                                                   pose.y - step_m * math.sin(pose.theta), pose.theta)
        return pose
...
def build_world_and_plan(cfg: ProjectConfig, app_cfg: AppConfig):
    ...
    if app_cfg.map_type == "LIDAR":
        print("[INFO] LIDAR mode: Will use real-time LiDAR data for mapping")
        print("[INFO] No predefined maze required; the robot will freely explore the real environment")
        ...
        local_cfg = RandomMazeConfig(
            size_m=cfg.random.size_m, cell_size_m=cfg.random.cell_size_m,
            random_wall_count=cfg.random.random_wall_count,
            random_seed=None, candidates_to_list=cfg.random.candidates_to_list,
            seed_scan_start=cfg.random.seed_scan_start,
            seed_scan_stride=cfg.random.seed_scan_stride, max_attempts_per_page=cfg.random.max_attempts_per_page,
            segment_len_cells_min=cfg.random.segment_len_cells_min,
            segment_len_cells_max=cfg.random.segment_len_cells_max,
...
def main() -> None:
    if not SIM_MODE:
        import  interface_51 as robot_serial
        import sys
        if len(sys.argv) > 1:
            port = sys.argv[1]
        else:
            try:
                port = input("Enter Bluetooth serial port (e.g., COM3 or /dev/rfcomm0): ").strip()
    ...
    if app_cfg.map_type == "LIDAR":
        ...
        print(f"[INFO] LIDAR mode: OGM range [{ogm_minx:.1f}, {ogm_miny:.1f}] ~ [{ogm_maxx:.1f}, {ogm_maxy:.1f}]")
    ...
    viz = MatplotlibViz(viz_size_m, world.cell_size_m, all_cfg.viz, robot_radius_m=all_cfg.robot.robot_radius_m,
                        ogm_bounds=ogm_bounds)
    ...
    if not SIM_MODE:
        ...
        if app_cfg.map_type == "LIDAR":
            ...
            print("[INFO] LIDAR mode: Using entrance as the starting position")
            x0, y0 = cell_center_m(entrance, world.cell_size_m)
            print("[INFO] LIDAR mode: Reading actual heading angle, but position is forced to use entrance")
            import robot_serial_interface_enhanced as robot_serial
            try:
                actual_pose = robot_serial.get_robot_pose(timeout=3.0)
                theta0 = actual_pose.theta
                print(f"[INFO] Read heading angle: {math.degrees(theta0):.1f}°")
            except Exception as e:
                print(f"[WARN] Failed to read heading angle: {e}, using default value 0°")
                theta0 = 0.0

            print(
                f"[INFO] 【Coordinate system alignment】Setting STM32 odometer initial position to entrance: ({x0:.3f}m, {y0:.3f}m)")
            try:
                if robot_serial.set_odometry_pose(x0, y0, theta0):
                    print(f"[OK] STM32 odometer initial position set")
                else:
                    print(f"[WARN] Failed to set STM32 odometer initial position, continuing to run")
            except Exception as e:
                print(f"[WARN] Unable to set STM32 odometer initial position: {e}")

            start_pose = Pose(x=x0, y=y0, theta=theta0)
            print(
                f"[INFO] Start point (forced entrance): ({start_pose.x:.3f}m, {start_pose.y:.3f}m, {math.degrees(start_pose.theta):.1f}°)")
            print(
                f"[INFO] ⚠️  Please ensure the robot is actually placed at entrance cell: ({entrance[0]}, {entrance[1]})")
            print(f"[INFO] Exploration target: From entrance {entrance} → exit {goal_cell}")
        else:
            # Predefined maze mode: Force to use entrance cell center
            x0, y0 = cell_center_m(entrance, world.cell_size_m)
            print("[INFO] Predefined maze mode: Reading actual heading angle, but position is forced to use entrance")
            import robot_serial_interface_enhanced as robot_serial
            try:
                actual_pose = robot_serial.get_robot_pose(timeout=3.0)
                theta0 = actual_pose.theta  # Use actual heading angle
                print(f"[INFO] Read heading angle: {math.degrees(theta0):.1f}°")
            except Exception as e:
                print(f"[WARN] Failed to read heading angle: {e}, using default value 0°")
                theta0 = 0.0
            ...
            start_pose = Pose(x=x0, y=y0, theta=theta0)
            print(
                f"[INFO] START(entrance): ({start_pose.x:.3f}m, {start_pose.y:.3f}m, {math.degrees(start_pose.theta):.1f}°)")
            print(f"[INFO] ⚠️ Put at entrance: ({entrance[0]}, {entrance[1]})")
    else:
        x0, y0 = cell_center_m(entrance, world.cell_size_m)
        theta0 = 0.0
        if len(global_path_xy) >= 2:
            x1, y1 = global_path_xy[1];
    ...
    CTX.sensor = sensor;
    CTX.pose = start_pose;
    CTX.robot_cfg = robot_cfg;
    CTX.ctrl_cfg = ctrl_cfg
    # Keyboard
